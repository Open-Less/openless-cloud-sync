...
更新：2026-09-26。**服务已部署到 `https://apic.openless.top:9443`，当前为指定账号联调模式。** 当前连接配置见 [连接信息](connection.md)，已执行证据见 [服务端验收](server-acceptance.md)。下文示例域名和默认端口用于通用部署说明。
...
2026-09-26 从官方仓库检出并构建提交 `40b519ae15ee5029578f377ff8baa8f25833e692`，安装为独立 `openless-sync` 用户运行的 systemd 服务。公网入口为 `https://apic.openless.top:9443`，使用已有域名证书；内部只监听 `127.0.0.1:8788`。防火墙新增允许 9443/tcp，未开放内部端口。

沿用现有 OpenLess OAuth App（Client ID 见连接信息），服务器凭据已经配置并经 GitHub 核验；没有创建第二个 OAuth App。当前 `restricted` 名单只包含 `appergb`，等待客户端完成后进行真实账号及加密同步联调。

主服务、每日备份和每小时清理定时器均已启用，首次备份与清理执行成功。配置和数据库权限为 0600；备份目录为 0700。官网及市场的既有 nginx 配置摘要与部署前一致，官网 HTTPS 仍返回 200。源码归档、许可证及 `/revision` 已经从公网访问验证。

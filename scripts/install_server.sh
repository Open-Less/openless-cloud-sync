...
if [[ ${EUID} -ne 0 || $# -ne 3 ]]; then
    echo 'Usage: sudo bash scripts/install_server.sh DOMAIN CERTIFICATE CERTIFICATE_KEY' >&2
    exit 2
fi
sync_domain=$1
sync_cert=$2
...
trap cleanup EXIT
python3 "$sync_source/scripts/render_nginx.py" "$sync_domain" "$sync_cert" "$sync_key" > "$sync_stage/nginx.new"
sync_activated=true
...
curl --fail --silent --show-error --retry 10 --retry-connrefused --retry-delay 1 --max-time 5 http://127.0.0.1:8787/healthz > /dev/null
systemctl reload nginx
curl --fail --silent --show-error --max-time 15 --resolve "$sync_domain:443:127.0.0.1" "https://$sync_domain/v1/capabilities" > /dev/null
curl --fail --silent --show-error --max-time 15 --resolve "$sync_domain:443:127.0.0.1" "https://$sync_domain/source.tar.gz" -o "$sync_stage/published-source.tar.gz"
...
echo "Deployment healthy: https://$sync_domain/v1/capabilities"

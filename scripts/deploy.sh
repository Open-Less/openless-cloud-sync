...
set -euo pipefail
if [[ $# -lt 4 || $# -gt 5 ]]; then
    echo 'Usage: bash scripts/deploy.sh SSH_HOST DOMAIN CERTIFICATE_PATH CERTIFICATE_KEY_PATH [COMMIT_SHA]' >&2
    exit 2
fi
sync_host=$1
sync_domain=$2
